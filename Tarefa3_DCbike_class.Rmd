...
subtitle: "Métodos Supervisionados de Classificação"
output: 
  html_document:
    toc: true
    toc_float: true
...
9.  Recrie a visualização abaixo, mostrando a relação entre o aluguel de bicicletas, a estação (`season`) e o tipo de dia (`workingday`). Interprete-a no contexto dos dados.

```{r eval = T, fig.retina=2}

plot2 <- dcbikeshare %>% 
  ggplot(aes(season,cnt, fill=workingday))+
...
12. Ajuste um modelo logístico prevendo o tipo de dia (`workingday`) a partir dos atributos presentes no dataset. Calcule o $AUC$, grafique a curva $ROC$, o `accuracy`, o `sensitivity`, e `specificity` e a matriz de confusão.

13. Ajuste um modelo de árvore de decisão prevendo o tipo de dia (`workingday`) a partir dos atributos presentes no dataset. Calcule o $AUC$, grafique a curva $ROC$, o `accuracy` , o `sensitivity`, e `specificity` e a matriz de confusão.

14. Utilize o algoritmo Random Forests para prever o tipo de dia (`workingday`) a partir dos atributos presentes no dataset. Calcule o $AUC$, grafique a curva $ROC$, o `accuracy`, o `sensitivity`, e `specificity` e a matriz de confusão.
...
15. Qual dos três modelos foi melhor? **dica**: compare os $AUC$, os `accuracy`, os `sensitivity`, e os `specificity` usando uma tabela.

16. Use o melhor modelo para calcular o $AUC$, graficar a curva $ROC$, calcular o `accuracy`, calcular o `sensitivity`, e calcular o `specificity` no dataset de teste. Grafique a curva $ROC$ final.
...
A escolha de atributos que devem compor um modelo de machine learning define, de certa forma, a qualidade das previsões. Neste sentido, a nota final da tarefa será ponderada de acordo ao ranking de melhores previsões, a partir da média aritmética entre as notas individuais de cada indicador no dataset de teste - da questão 16 - (AUC, accuracy, sensitivity e specificity) e organizada por percentis:
