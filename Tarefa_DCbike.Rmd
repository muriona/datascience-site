...
Nesta segunda tarefa usaremos um dataset relacionado com o serviço de aluguel de bikes em Washington DC, EUA, com registros por hora de aluguel de bikes para o período 2011 a 2012.
...
dcbikeshare <- read.csv("data_bikesharing/hour.csv")
...
xfun::embed_dir("data_bikesharing/", text = "Você pode baixar os dados aqui.")
```


As fontes de dados originais se encontram em: <http://capitalbikeshare.com/system-data> e <http://www.freemeteo.com>.

O dataset chama-se `hour.csv`. A descrição de cada uma das variáveis pode ser encontrada em [Kaggle](https://www.kaggle.com/datasets/marklvl/bike-sharing-dataset).
...
8.  Recrie a seguinte visualização, e interprete-a no contexto dos dados. **Dica**: Você precisará usar uma das variáveis que você criou acima. A temperatura plotada é a `sensação térmica`. Antes de realizar o gráfico verifique que o tipo da variável `dteday` deve ser `Date` e não `chr`. Agrupe os dados por dia.
...
17. Escreve suas conclusões com base nos resultados mais relevantes encontrados.

## Entrega

Para esta tarefa a entrega incluirá:

* O arquivo `.Rmd` a ser entregue pelo moodle.
* A versão 'funcional' a ser entregue via compartilhamento com o Professor no Workspace. 

## Avaliação

A avaliação levará em conta a sua capacidade de trabalhar com o `tidymodels` assim como com os outros pacotes necessários para realizar a atividade, de forma a construir modelos de machine learning que tragam *insights* relevantes sobre o uso de bike sharing.
